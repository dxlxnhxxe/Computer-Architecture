...
void add_to_table(SymbolTable *table, const char *name, const char *type, VariableKind kind) {
    ...
    new->kind = kind;
    new->offset = table->num_allocated[kind];
    table->num_allocated[kind]++;
...
bool is_primitive(TableEntry *entry) {
    return (strcmp(entry->type, "int") == 0 || strcmp(entry->type, "char") == 0|| strcmp(entry->type, "boolean") == 0);
}